...
def right_drm_indiv_trim(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Individual Vh trim with renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
    ...
    for i, Vh in enumerate(Vhs):
        ...
        Vhs[i] = zero_out_non_topk(Vhs[i], ratio=1 - basis_dropping_rate)
    ...
    Vhs = torch.stack(Vhs, dim=0)
    SVhs = Ss.unsqueeze(-1) * Vhs
    SVhs = resolve_elementwise_sign_conflict(SVhs, resolve_method="mass", dim=0, resolute_sign=None)
    SVh = average(SVhs, disjoint=enable_disjoint_mean, dim=0)
    return U @ SVh


def left_drm_indiv_trim(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Individual U trim with renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
    ...
    Us = list(torch.split(U, split_size_or_sections=[n] * num_models, dim=0))
    Ss = []
    for i, current_U in enumerate(Us):
        U_norms = torch.linalg.vector_norm(current_U, ord=2, dim=0, keepdim=True)

        # Normalize right singular vectors
        current_U /= U_norms

        Us[i] = zero_out_non_topk(Us[i], ratio=1 - basis_dropping_rate)
    ...
    Us = torch.stack(Us, dim=0)  # (num_models, n, r)
    UsS = Ss.unsqueeze(-2) * Us  # (num_models, 1, r) * (num_models, n, r)
    UsS = resolve_elementwise_sign_conflict(UsS, resolve_method="mass", dim=0, resolute_sign=None)
    US = average(UsS, disjoint=enable_disjoint_mean, dim=0)
    return US @ Vh


def right_drm_joint_trim_norenorm(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Global Vh trim witout renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
    ...
    # Calculate rank as per torch.linalg.matrix_norm TODO
    # rank_threshold = S.max() * max(n, m) * torch.finfo(S.dtype).eps
    # logging.debug(f'Rank threshold = {rank_threshold}, having highest svdval of {S.max().item()}.')
    rank_threshold = S.max().item() * 1e-2
    ranks = (S > rank_threshold).sum().item()

    print("Individual shape:", n, m)
    print("Stacked shape:", col_concat_weights.shape)
    print("Ranks =", ranks)
    print()
    ...
    Vh = zero_out_non_topk(Vh, ratio=1 - basis_dropping_rate)
    Vhs = list(torch.split(Vh, split_size_or_sections=[m] * num_models, dim=1))
    Vhs = torch.stack(Vhs, dim=0)
    SVhs = torch.diag_embed(S).unsqueeze(0) @ Vhs
    SVhs = resolve_elementwise_sign_conflict(SVhs, resolve_method="mass", dim=0, resolute_sign=None)
    SVh = average(SVhs, disjoint=enable_disjoint_mean, dim=0)
    return U @ SVh


def left_drm_joint_trim_norenorm(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Global U trim witout renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
    ...
    ranks = (S > rank_threshold).sum().item()
    print("Individual shape:", n, m)
    print("Stacked shape:", row_concat_weights.shape)
    print("Ranks =", ranks)
    print()
    ...
    U = zero_out_non_topk(U, ratio=1 - basis_dropping_rate)
    Us = list(torch.split(U, split_size_or_sections=[n] * num_models, dim=0))
    Us = torch.stack(Us, dim=0)
    UsS = Us @ torch.diag_embed(S).unsqueeze(0)
    UsS = resolve_elementwise_sign_conflict(UsS, resolve_method="mass", dim=0, resolute_sign=None)
    UsS = average(UsS, disjoint=enable_disjoint_mean, dim=0)
    return UsS @ Vh


def right_drm_indiv_trim_norenorm(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Individual Vh trim witout renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
...
def left_drm_indiv_trim_norenorm(
    weights: List[torch.Tensor],
    base_weight: torch.Tensor,
    basis_dropping_rate: float = 0.0,
    enable_disjoint_mean: bool = True,
):
    """Individual U trim witout renormalization."""
    if basis_dropping_rate == 0.0:
        return weights
